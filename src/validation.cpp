...
#if defined(NDEBUG)
# error "Scribe Core cannot be compiled without assertions."
#endif
...
/** Fees smaller than this (in nSCRIBE) are considered zero fee (for relaying, mining and transaction creation) */
...
const string strMessageMagic = "ScribeCoin Signed Message:\n";
...
CAmount GetBlockSubsidy(int nPrevBits, int nPrevHeight, const Consensus::Params& consensusParams, bool fSuperblockPartOnly)
{
    ...
    // yearly decline of production by 10% per year until reached max coin
...
{
    CAmount ret = blockValue/2.5; // start at 40%

    int nMNPIBlock = Params().GetConsensus().nMasternodePaymentsIncreaseBlock;
    int nMNPIPeriod = Params().GetConsensus().nMasternodePaymentsIncreasePeriod;

                                                                      // mainnet:
    if(nHeight > nMNPIBlock)                  ret += blockValue / 20; // 40320  - 45.0%
    if(nHeight > nMNPIBlock+(nMNPIPeriod* 1)) ret += blockValue / 20; // 53760  - 50.0%
    if(nHeight > nMNPIBlock+(nMNPIPeriod* 2)) ret += blockValue / 20; // 67200  - 55.0%
    if(nHeight > nMNPIBlock+(nMNPIPeriod* 3)) ret += blockValue / 20; // 80640  - 60.0%
    if(nHeight > nMNPIBlock+(nMNPIPeriod* 4)) ret += blockValue / 20; // 94080  - 65.0%
...
static bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck = false)
{
    ...
    // SCRIBE : MODIFIED TO CHECK MASTERNODE PAYMENTS AND SUPERBLOCKS
    ...
    if (!IsBlockValueValid(block, pindex->nHeight, blockReward, strError)) {
        return state.DoS(0, error("ConnectBlock(SCRIBE): %s", strError), REJECT_INVALID, "bad-cb-amount");
    }

    if (!IsBlockPayeeValid(block.vtx[0], pindex->nHeight, blockReward)) {
        mapRejectedBlocks.insert(make_pair(block.GetHash(), GetTime()));
        return state.DoS(0, error("ConnectBlock(SCRIBE): couldn't find masternode or superblock payments"),
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot)
{
    ...
    // SCRIBE : CHECK TRANSACTIONS FOR INSTANTSEND
    ...
    if(sporkManager.IsSporkActive(SPORK_3_INSTANTSEND_BLOCK_FILTERING)) {
        ...
        BOOST_FOREACH(const CTransaction& tx, block.vtx) {
            ...
            BOOST_FOREACH(const CTxIn& txin, tx.vin) {
                ...
                if(instantsend.GetLockedOutPointTxHash(txin.prevout, hashLocked) && hashLocked != tx.GetHash()) {
                    ...
                    LOCK(cs_main);
                    mapRejectedBlocks.insert(make_pair(block.GetHash(), GetTime()));
                    return state.DoS(0, error("CheckBlock(SCRIBE): transaction %s conflicts with transaction lock %s",
        ...
        LogPrintf("CheckBlock(SCRIBE): spork is off, skipping transaction locking checks\n");
    }
