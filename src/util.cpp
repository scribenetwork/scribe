...
const char * const BITCOIN_CONF_FILENAME = "scribe.conf";
const char * const BITCOIN_PID_FILENAME = "scribed.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "scribe" is a composite category enabling all Scribe-related debug output
            if(ptrCategory->count(string("scribe"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\ScribeCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\ScribeCore
    // Mac: ~/Library/Application Support/ScribeCore
    // Unix: ~/.scribecore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "ScribeCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/ScribeCore";
#else
    // Unix
    return pathRet / ".scribecore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty scribe.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override scribe.conf
